...
1. Start with one strong <h1> title in Chinese.
...
Build a Chinese mind map from this transcript.
Video title: ${video.title}
...
Rules:
- 4-7 top-level nodes.
- Each node summary must be one concise Chinese sentence.
...
Rules:
- Return 3-8 people.
- Use concise Simplified Chinese for role and relevance.
...
- Reasons should be concise Chinese sentences.
...
export function buildPersonDetailPrompt({personName, video, transcriptEntries}) {
  const transcript = serializeTranscriptForPrompt(transcriptEntries, 10000);
  return `
Return valid JSON only. No markdown fences.

Create a short Chinese detail card for the person below in the context of this video.
